...
def output_bulk_errors(errors):
    for error in errors:
        ...
        if error.editorial_reason_code is not None:
            ...
            output_status_message(
                "EditorialLocation: {0}".format(error.editorial_location)
            )
            output_status_message(
                "PublisherCountries: {0}".format(error.publisher_countries)
            )
        output_status_message("")
...
def output_bulk_quality_score_data(quality_score_data):
    if quality_score_data is not None:
        output_status_message(
            "KeywordRelevance: {0}".format(quality_score_data.keyword_relevance)
        )
        output_status_message(
            "LandingPageRelevance: {0}".format(
                quality_score_data.landing_page_relevance
            )
        )
        output_status_message(
            "LandingPageUserExperience: {0}".format(
                quality_score_data._landing_page_user_experience
            )
        )
        output_status_message(
            "QualityScore: {0}".format(quality_score_data.quality_score)
        )
...
def output_bulk_product_partitions(bulk_ad_group_product_partitions):
    ...
    for bulk_ad_group_product_partition in bulk_ad_group_product_partitions:
        ...
        if partition.ParentCriterionId is not None:
            child_branches[partition.ParentCriterionId].append(
                bulk_ad_group_product_partition
            )
        else:
...
def output_bulk_product_partition_tree(node, child_branches, tree_level):
    ...
    for i in range(0, tree_level):
        pad = pad + "\t"
    output_status_message(
        "{0}{1}".format(pad, ad_group_criterion.Criterion.PartitionType)
    )

    output_status_message(
        "{0}ParentCriterionId: {1}".format(
            pad, ad_group_criterion.Criterion.ParentCriterionId
        )
    )

    output_status_message("{0}Id: {1}".format(pad, ad_group_criterion.Id))
    ...
    null_attribute = (
        "(All other)"
        if ad_group_criterion.Criterion.ParentCriterionId is not None
        else "(Tree Root)"
    )
    output_status_message(
        "{0}Attribute: {1}".format(
            pad,
            null_attribute
            if ad_group_criterion.Criterion.Condition.Attribute is None
            else ad_group_criterion.Criterion.Condition.Attribute,
        )
    )
